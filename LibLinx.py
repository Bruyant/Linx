...
def data_analyze(data, RefData, thefreq, SampleRate, cutoff=None,
    ...
    fit_le = int(Phase_accuracy * round(sample_rate / thefreq))
    ...
    # lowpass filtering
    sig_out[:, 0] = data_lpass(sig_out[:, 0], cutoff, sample_rate)
    ...
    # lowpass filtering
    sig_out[:, 1] = data_lpass(sig_out[:, 1], cutoff, sample_rate)
...
if __name__ == '__main__':
    ...
    # test of demodultion
    #signal creation
    ModAmp=(1 + 0.1 * sin(data[:, 0] * 20 * 2 * pi))
